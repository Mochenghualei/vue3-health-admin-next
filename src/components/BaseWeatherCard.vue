<template>
  <div class="container" ref="container">
    ...
    <div id="card" class="weather" ref="card">
      <svg id="inner" ref="inner">
        <defs>
          <path id="leaf"
      ...
      <div class="details">
        ...
        <div class="right">
          <div id="date" ref="date">{{ formatted }}</div>
          <div id="summary" ref="summary">{{ weatherNow }}</div>
        </div>
      </div>
    </div>
    <svg id="outer"></svg>
...
onMounted(() => {
  ...
  innerRainHolder1 = innerLeafHolder = innerSnowHolder = innerLightningHolder = weatherContainer1.group()
  ...
  clouds = [{ group: Snap.select('#cloud1') }, { group: Snap.select('#cloud2') }, { group: Snap.select('#cloud3') }]
})
...
function updateSummaryText() {
  TweenMax.fromTo(summary.value, 1.5, { x: 30 }, { opacity: 1, x: 0, ease: Power4.easeOut })
...
function makeRain() {
  ...
  var line = innerRainHolder['innerRainHolder' + (3 - Math.floor(lineWidth))].path('M0,0 0,' + lineLength).attr({
    fill: 'none',
